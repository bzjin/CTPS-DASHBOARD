...
queue()
	...
	.awaitAll(function(error, results){ 
		...
		CTPS.demoApp.generateNonInterstateYears(results[0], results[1], results[2], results[3], results[4], results[5], results[6], results[7], results[8]);
...
CTPS.demoApp.generateNonInterstateYears = function(yr2007, yr2008, yr2009, yr2010, yr2011, yr2012, yr2014, yr2015, townids){
	...
	var pushed = [];
	var resultsarray = [yr2007, yr2008, yr2009, yr2010, yr2011, yr2012, yr2012, yr2014]
		...
		nested_array.forEach(function(i){ 
			nested_quartiles.forEach(function(j) { 
				if (i.key == j.key) { //find matching towns
					...
					if (j.values.length > 1) {
						for (var k = 0; k < j.values.length-1; k++) { //scroll through cumulative lengths
							...
							if (j.values[k].properties.cumulative < i.values.total/4) {
								firstStorage.push(((j.values[k].properties.PSI * roadlength) + (j.values[k+1].properties.PSI * nextRoadLength))/(roadlength + nextRoadLength));
								firstQuartile = d3.max(firstStorage);
							}  if (j.values[k].properties.cumulative < i.values.total/2) {
								medStorage.push(((j.values[k].properties.PSI * roadlength) + (j.values[k+1].properties.PSI * nextRoadLength))/(roadlength + nextRoadLength));
								median = d3.max(medStorage);	
							} if (j.values[k].properties.cumulative < i.values.total*3/4) {
								thirdStorage.push(((j.values[k].properties.PSI * roadlength) + (j.values[k+1].properties.PSI * nextRoadLength))/(roadlength + nextRoadLength));
					...
					if (i.values.thirdQuartile == 0 || isNaN(i.values.thirdQuartile)) { i.values.thirdQuartile = i.values.avgtotal/i.values.total}
					if (i.values.median == 0 || isNaN(i.values.median)) { i.values.median = i.values.thirdQuartile }
					if (i.values.firstQuartile == 0 || isNaN(i.values.firstQuartile)) { i.values.firstQuartile = i.values.median}
				}
			})
		...
		nested_array.forEach(function(i){ 
			nested_quartiles.forEach(function(j) { 
				if (i.key == j.key) { //find matching towns
					...
					if (i.values.thirdQuartile == 0) { i.values.thirdQuartile = i.values.avgtotal/i.values.total}
					if (i.values.median == 0) { i.values.median = i.values.thirdQuartile }
					if (i.values.firstQuartile == 0) { i.values.firstQuartile = i.values.median}
