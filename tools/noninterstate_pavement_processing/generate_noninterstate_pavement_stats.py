...
#                filterd to contain only records with 
...
#
# The PSI buckets for non-interstate roads are defined as follows:
#       1. Poor:      PSI 0.0 to 2.3
#       2. Fair:      PSI 2.3 to 2.8
#       3. Good:      PSI 2.8 to 3.5
...
# Step 4 - select records with PSI < 2.3, calc number of lane miles, and clear selection.
arcpy.SelectLayerByAttribute_management(in_layer_or_view=ri_table_name,selection_type="NEW_SELECTION",where_clause="PSI IS NOT NULL AND PSI < 2.3")
...
# Step 5 - select records with PSI >= 2.3 AND PSI < 2.8, calc number of lane miles, and clear selection.
arcpy.SelectLayerByAttribute_management(in_layer_or_view=ri_table_name,selection_type="NEW_SELECTION",where_clause="PSI IS NOT NULL AND PSI >= 2.3 AND PSI < 2.8")
...
# Step 6 - select records with PSI >= 2.8 AND PSI < 3.5, calc number of lane miles, and clear selection.
arcpy.SelectLayerByAttribute_management(in_layer_or_view=ri_table_name,selection_type="NEW_SELECTION",where_clause="PSI IS NOT NULL AND PSI >= 2.8 AND PSI < 3.5")
